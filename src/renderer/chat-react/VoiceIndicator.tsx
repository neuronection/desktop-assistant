...
interface VoiceIndicatorProps {
  ...
  level: number;
  interim?: string;
  /** The shown text is the last phrase that was sent (live mode) — render a delivered check. */
  sent?: boolean;
  onCancel?: () => void;
  onClearInterim?: () => void;
}
...
export function VoiceIndicator(props: VoiceIndicatorProps): JSX.Element | null {
  ...
  if (props.state === 'idle' && !props.sent) {
    return null;
  }
      ...
      {props.interim ? (
        <div ref={bodyRef} className="max-h-24 overflow-y-auto whitespace-pre-wrap break-words text-xs leading-relaxed opacity-80">
          ...
          {props.sent ? (
            <span
              role="img"
              aria-label={TEXT.VOICE_SENT}
              className="ml-1.5 inline-flex align-text-bottom text-[var(--as-primary)]"
            >
