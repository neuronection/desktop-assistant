...
const formatSchema = z.object({
  ...
  date: z.union([z.string(), z.number()]).optional().describe("Date to operate on: ISO 8601 string, epoch seconds/millis, 'now', or 'today'. Defaults to now where a date is needed."),
  to: z.union([z.string(), z.number()]).optional().describe('Second date for diff/relative (defaults to now).'),
