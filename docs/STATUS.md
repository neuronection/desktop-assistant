...
| Decision engines (plan 20 S1–S3): `AiTask.INTENT` audit task, `config.decision` (engine `off`/`llm`/`needle` + act/confirm thresholds, default OFF = pre-plan behavior), decision funnel `src/main/ai/decide/` (resolve → invoke → zod-validate → `AiCall` audit → act/confirm/refuse band; every non-decided status falls through to the agent path), LLM structured-output engine over a new `createStructuredChatModel` factory seam; local Needle engine (ADR-0019): vendored pinned wasm runtime (`resources/needle/`, utilityProcess transport with serialized ops + timeouts), weights manager (user-initiated 35 MB download, size+sha256 verified, atomic rename, offline after), zod-validated parsing (unknown tools are errors); turn fast path (S3): short plain inputs dispatch through the direct-tool turn (same policy/approvals, confirm band forces the card, provenance in message metadata, tool-surface projection native zod→JSON + app MCP, capped at 40) — refuse/multi/error always fall through. Settings UI is S5; engines enable via config only for now | done (S3) |
