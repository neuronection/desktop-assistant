...
| Tool apps settings (plan 15 S5): Apps tab (list + health chips + search/filters, detail modal with connection editor / per-tool table with undo + tighten-only risk / D18 scope editor with `apps:preview-scope` device preview / exposure gated on `deferredSupported`), preset permission-preview card, budget card (`config.toolApps.toolBudget` enforced by selection), Tools-tab MCP section retired into Apps, axe scans exclusion-free | done (S1–S5 of 6; palette ride-along + docs sweep pending) |
