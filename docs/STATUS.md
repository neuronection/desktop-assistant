...
| Decision engines (plan 20 S1): `AiTask.INTENT` audit task, `config.decision` (engine `off`/`llm`/`needle` + act/confirm thresholds, default OFF = pre-plan behavior), decision funnel `src/main/ai/decide/` (resolve → invoke → zod-validate → `AiCall` audit → act/confirm/refuse band; every non-decided status falls through to the agent path), LLM structured-output engine over a new `createStructuredChatModel` factory seam; needle local engine + turn routing + settings UI are later stages — no caller yet | done (S1) |
