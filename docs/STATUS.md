...
| TTS replies (plan 12 S6): `tts` task assignment (audio-capped models) + Settings → Voice "Speak replies" (default off) with voice/speed, sanctioned `/audio/speech` module audited on the `tts` task, markdown-stripped speakable text, `da-voice-wave` speaking bar with stop in both windows, hidden-window playback + unchanged notifications | done |
